...
import requests
# Self-contained cache-path resolution (this script lives in kayak_data ops/,
# away from the engine's scripts/_gauge_metadata_cache.py): $GAUGE_METADATA_CACHE
# wins; the fallback assumes the sibling-clone convention (kayak_data next to
# the kayak engine checkout — true on both the dev box and prod).
import os as _os
from pathlib import Path as _Path

DEFAULT_GAUGE_METADATA_CACHE = _Path(
    _os.environ.get(
        "GAUGE_METADATA_CACHE",
        str(_Path(__file__).resolve().parents[2] / "kayak" / "Gauge-metadata-cache" / "gauges.db"),
    )
)
